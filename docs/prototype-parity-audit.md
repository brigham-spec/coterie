...
| 10 | Second-degree contacts from meeting transcripts | Names in action items not in CRM surface as "+ Add to CRM" chips. (~L5528) | DONE (S11Z10: profile-scoped port — the "People from meetings" card (`_second-degree-card.tsx`) lists Fireflies attendees whose `meetingIds` intersect this company's meetings but match no contact; each chip offers "Add to CRM" (reuses `attachConnection(id, companyId)`) + dismiss (`dismissConnection`), then `router.refresh()`. Loader query in companies/[id]/page.tsx (`unmatchedAttendee.hasSome(meetingIds)`, dismissed filtered). The org-wide capability already shipped as the dashboard **New Connections** panel) | Yes (creates contacts) | L |
...
| 7 | Invoice Schedule spreadsheet | Full invoice schedule grid. | DONE (S11Z14) — dedicated `/dashboard/invoices/grid` billing-calendar view: rows=companies, 12 month columns grouped into quarters, each cell rolls up invoices DUE that month by derived status (overdue > open > paid), with month/company/grand totals and a zero-JS `?year=YYYY` picker. PURE `buildInvoiceGrid` (src/lib/invoice-grid.ts) reads the same Invoice+payment ledger (void excluded) so grid and ledger can never disagree; cross-linked from invoices/page.tsx. Read-only (settle on the invoice detail). Ledger + detail (create/recordPayment/voidInvoice) already shipped. | Derived | L |
| 8 | Daily Focus / My Agenda | AI-Prep, per-item done/snooze/waiting states. | DONE (S11Z12) — per-item done/snooze/waiting triage on the Daily Focus card. New `AgendaItemState` store (schema.prisma; migration 20260810000000_slice_agenda_item_states, org-scoped + RLS) keyed polymorphically by (kind, refId) — the durable actionItem/event id behind each ephemeral focus item, so a mark survives regeneration. `applyAgendaStates` (lib/agenda-state.ts, pure) folds the overlay onto built items (done + unexpired-snooze drop, waiting tagged); `setAgendaItemState` action (daily-focus-actions.ts) upserts/clears (AI-free); FocusRow (_daily-focus.tsx) renders Done/Snooze/Waiting buttons with optimistic overrides + session Undo. Org-scoped (shared triage, matching how the focus draws every open commitment/event). | **Yes** (agenda item state — applied) | M |
...
| 16 | Persistent Dismiss + reason taxonomy | Dismiss saves to store (org never re-surfaces) w/ 4 reasons (not_relevant/already_connected/competitor/wrong_timing). (~L15292) | DONE (S11Z1) — ProspectDismissal store (migration 20260808000000) + dismissProspect upsert keyed by normalized org name; Dismiss button reveals a 4-reason picker (PROSPECT_DISMISS_REASONS, src/lib/prospect-dismissal.ts); findProspects merges dismissed names into excludeOrgs so parseProspectTargets drops them on the next scan | Yes (dismissal store — migrated) | M |
