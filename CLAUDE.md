...
9. 🔶 Security review — internal audit + report done (docs/security-review.md; tenant isolation holds, no critical). Fixed H1 (HTTP security headers in next.config.ts, CSP report-only) + M2 (documented Inngest keys). M1 (RBAC) ACCEPTED as flat for the pilot — every org member is equal; the admin/staff role field is resolved but NOT yet an authorization boundary (revisit before broader access). M3 (open tenant self-provisioning) ACCEPTED as open for the pilot — app-side provisioning stays open; the effective gate is upstream Clerk sign-up (must stay invite-only/restricted; revisit with an allowlist before provisionOrg if Clerk sign-up ever goes public). Optional: external pentest before GA.
10. ⬅ NEXT: HVEDC pilot
